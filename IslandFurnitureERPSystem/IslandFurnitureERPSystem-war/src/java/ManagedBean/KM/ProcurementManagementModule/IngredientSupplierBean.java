...
public class IngredientSupplierBean implements Serializable {
    ...
    public void init() {
        try {
            ...
            System.err.println("ManagedBean.KM..ProcurementManagementModule.IngredientSupplierBean: init(): Failed. Caught an unexpected exception.");
            ex.printStackTrace();
        }
        filteredSuppliers = pm.getSuppliers(kitchen.getId());
    }

    public void addSupplier(ActionEvent event) {
        Long supplierId = pm.addSupplier(kitchen.getId(), name, address, contact, fax, remark);
        if (supplierId == -1L) {
    ...
    public void onRowEdit(RowEditEvent event) {
        ...
        try {
            Long temp = pm.editSupplier(supplier.getId(), supplier.getName(), supplier.getAddress(), supplier.getContact(), supplier.getFax(), supplier.getRemark());
            if (temp == -1L) {
            ...
            FacesMessage msg = new FacesMessage("Edition Faild", "Unexpected Exception Occurred");
            FacesContext.getCurrentInstance().addMessage(null, msg);
            System.err.println("ManagedBean.KM.ProcurementManagementModule.IngredientSupplierBean: onRowEdit(): Failed. Caught an unexpected exception.");
            ex.printStackTrace();
        }
        filteredSuppliers = pm.getSuppliers(kitchen.getId());
    }
    ...
    public void deleteSupplier(IngredientSupplierEntity supplier) {
        try {
            Long temp = pm.deleteSupplier(supplier.getId());
            if (temp == -1L) {
            ...
            System.err.println("ManagedBean.KM.ProcurementManagementModule.IngredientSupplierBean: deleteSupplier(): Failed. Caught an unexpected exception.");
            ex.printStackTrace();
        }
        filteredSuppliers = pm.getSuppliers(kitchen.getId());
    }
